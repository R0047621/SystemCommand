...
http://192.168.122.36:1024${item.cover}`"
        ...
        <div class="w-[200px] mt-2 mr-[50px]">
          <p class="text-[14px] text-[#00000073]">进度</p>
          <a-progress :percent="item.progress" size="small" />
        </div>

        <!-- 字体图标部分 -->
        <div class="w-[160px] flex items-center justify-between mt-3">
          <a-tooltip
            placement="top"
  ...
  <a-modal v-model:open="open" title="项目成员" width="530px" @ok="handleOk">
    <a-space direction="vertical">
      <a-input placeholder="请输入用户名或邮箱查找" />
    </a-space>
  </a-modal>
  <a-pagination
...
import { getProjectsData } from "@/service/index";
import { ref } from "vue";
